...
explanations = {"horas_frio": "Horas de frio, si no se " \
    ...
    "penman_monteith_fao": "Milímetros de agua evaporada en un día por una " \
    ...
    if phenologic_calc == "chill":
        ...
        if kwdata["chosen_method"] == "penman_monteith_fao":        
            ...
            relative_distance = 1 + 0.033 * math.cos((2 * math.pi / 365) \
                *julian_date)
            solar_decline = 0.409 * math.sin((2 * math.pi / 365) * \
                julian_date - 1.39)
            ...
            extrat_rad = ((24*60)/math.pi)*0.082*relative_distance* \
                (sun_angle*math.sin(lat_rad)*math.sin(solar_decline)+ \
                math.cos(lat_rad)*math.cos(solar_decline)*math.sin(sun_angle))
            ...
            sat_vapor_pressure = 0.6108*math.exp((17.27*mid_temp)/(mid_temp \
                + 237.3))
            ...
            net_long_rad = 4.903e-9*((max_temp**4 + min_temp**4)/2)*(0.34 \
                -0.14*math.sqrt(vapor_pressure))*(1.35*(solar_rad/clear_day_rad) \
                -0.35)
            ...
            result = (0.408*curve_slope*net_radiation+psicometric_constant* \
                (900/(mid_temp+273))*wind_speed*pressure_deficit)/(curve_slope+ \
                psicometric_constant*(1+0.34*wind_speed))
